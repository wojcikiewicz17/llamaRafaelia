...
/* ZERAMENTO MANUAL (SUBSTITUI memset) */
void zero_metrics(NeuroMetrics* m) {
    u8* p = (u8*)m;
    for (u64 i = 0; i < sizeof(NeuroMetrics); i++) {
        p[i] = 0;
    }
}
...
void _start() {
    ...
    _sc(SYS_WRITE, STDOUT,
        (long)"[OMEGA V4B MONOLITH ACTIVE]\n", 28);
