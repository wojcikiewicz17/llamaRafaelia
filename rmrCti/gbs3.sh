...
static void bbs_menu(void){
  puts("");
  puts("╔══════════════════════════════════════════════════════════════╗");
  puts("║  GBS3 :: TRIAD_LAB + RAF_FIBO + ARENA (C)                    ║");
...
static int irand(int a, int b){ // inclusive
...
static void arena_draw(char g[GH][GW+1], Ent *P, Ent A[NAI], int tick, double deltaP){
  ...
  // obstacles (simple static pattern)
  ...
  // print
  printf("\n");
  printf("ARENA tick=%d  ΔP=%+.6f  (WASD move, q exit)\n", tick, deltaP);
  for(int y=0;y<GH;y++) puts(g[y]);
}
...
static int fibo_gate_step(int t){
  // usa 01123 + 001123 misturado p/ produzir gate 0..3
  // (âncora discreta, não “Fibonacci clássico” — é teu alfabeto)
  int s1[]={0,1,1,2,3};
  int s2[]={0,0,1,1,2,3};
  int a = s1[t%5];
  int b = s2[t%6];
  int j = (a + 2*b + (t%4)) & 3; // 0..3
  return j;
}

static void ai_update(Ent *e, Ent *P, int jgate, double deltaP){
  // deltaP alto => mais pressão (persegue mais reto)
  // jgate define “modo”: 0=repulsão, 1=persegue X, 2=persegue Y, 3=circula
  ...
  // small noise so it doesn't lock
  if((xorshift32() & 7u)==0u){
    mvx += irand(-1,1);
    mvy += irand(-1,1);
  }
...
static void arena_run(const char *root){
  ...
  while(1){
    // draw (also builds obstacles/borders)
    ...
    for(int i=0;i<NAI;i++){
      ...
      // collision with walls/obstacles -> bounce
